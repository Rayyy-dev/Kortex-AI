        ...
        if (isVisible) {
            const runProgress = () => {
                timer = setInterval(() => {
                    setProgress(prev => {
                        if (prev >= 100) {
                            // When reaching 100%, wait 5 seconds then start over
                            clearInterval(timer);
                            setTimeout(() => {
                                setProgress(0);
                                runProgress(); // Start the progress again
                            }, 5000);
                            return 100;
                        }
                        return prev + 1;
                    });
                }, 60); // 6 seconds to complete (60ms * 100)
            };

            runProgress(); // Start the initial progress
        }
