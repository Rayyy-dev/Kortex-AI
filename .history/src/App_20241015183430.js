  ...
  useEffect(() => {
    const animateValue = (id, start, end, duration) => {
      ...
      const step = (timestamp) => {
        ...
        document.getElementById(id).textContent = id === 'speed-value' ? `${value}ms` : `${value}%`;
        if (progress < 1) {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            animateValue('accuracy-value', 0, 98, 2000);
            animateValue('speed-value', 0, 50, 2000);
            animateValue('efficiency-value', 0, 95, 2000);
      ...
      <section className="performance-metrics">
        ...
        <div className="graph-container">
          <div className="metric-cards">
            ...
            <div className="metric-card">
              <h3>Efficiency</h3>
              <div className="metric-value" id="efficiency-value">0%</div>
            </div>
          </div>
